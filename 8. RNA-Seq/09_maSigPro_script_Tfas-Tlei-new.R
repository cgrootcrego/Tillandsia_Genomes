...
# Here we trim lowly-expressed genes. This doesn't change the results much but vastly shortens run time.
# Every gene with a mean cpm expression under 1 is eliminated.
...
# Median centering for visualization - not used in maSigPro
rowmed <- apply(normd_trim,1,median)
normd_medcentered <- normd_trim - rowmed
# Take the log2 of the mediancentered counts
normd_medcentered[normd_medcentered == 0] <- 1
normd_medcentered_log <- log2(abs(normd_medcentered))
write.table(normd_medcentered_log, file = "counts.Tfas_Tlei_6_timepoints.exons.sum.normalized-cpm.EdgeR.logtransformed.mediancentered.txt", sep = "\t", quote = F)

# Take the log2 without median-centering of the trimmed counts
