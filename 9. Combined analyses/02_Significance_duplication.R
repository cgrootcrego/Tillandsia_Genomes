setwd('/Users/clara/Documents/GitHub/Tillandsia-compgenomics/7. Rna-seq experiment 6 timepoints/Co-expression_networks_MaSigPro/mapped_to_Tfas/duplication_stats/')
...
# For Tlei stats
counts <- read.table("../../mapped_to_Tlei/duplication_stats.per-orthogroup_EXONIC_absolute.txt", sep = ' ', header = T, row.names = 1)
counts <- as.data.frame(t(counts))
contingency <- as.data.frame(counts$all, row.names = rownames(counts))
contingency$non_dge <- counts$whole_genome - counts$all
colnames(contingency) <- c("dge", "non_dge")
